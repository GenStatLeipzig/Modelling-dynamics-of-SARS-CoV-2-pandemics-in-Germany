...
maxdate_diagrams = as_date('2024-03-31')
...
plot_testpos_infectivity_allele = function(myintegrated_data, maxdate_diagrams) {
  ...
  p_fitage_reported =ggplot(myintegrated_data$plotdat_modelfit[agegroup2 %nin% c("unbekannt") & datum <= maxdate_diagrams& outcome =="reported"], aes(datum, value,col = agegroup2,  lty = type)) + 
    ...
    scale_x_date(breaks = date_breaks(width = "6 month"), labels =  date_format("%b-%y"))+
    xlab("") +
    ...
    theme_hc(base_size = 14) + ylab("Number") +
  ...
  # add variants ----
  p_var = ggplot(myintegrated_data$data$variantdata_outputlayer, aes(datum, dataVariant_anteil, col = variant2, lty = type)) + geom_line(lwd = 1) +     
    theme_hc(base_size = 14) +
    ...
    # annotate("text",x = myintegrated_data$datesVariant5proz_model$date+7, y = rep(1.1, nrow(myintegrated_data$datesVariant5proz_model)),label = myintegrated_data$datesVariant5proz_model$variant2, angle = 90)  +
    guides(lty = "none") + labs(color = "")
  p_var
  
  
  p_testpos_detail = p_fitage_reported / (p_infectivity +scale_y_continuous(labels = label_comma(accuracy = 0.1))) + p_var+ patchwork::plot_layout(heights = c(3.5,1, 1)) + plot_annotation(title = myintegrated_data$region)
...
# p_fit_ger =infectivityplot_DE/ severityplot_DE + plot_layout(nrow = 4,guides = 'collect', heights = c(5,1.7,2,6)) + plot_annotation(tag_levels = "A") & theme(legend.position = "top",plot.tag = element_text(face = 'bold', size = 19)) & theme_hc(base_size = 20)

infectivityplot_DE

pdf(here("results//Figure 3 testpos infectiv_Germany.pdf"), height = 8.5,width = 12)
infectivityplot_DE
dev.off()

pdf(here("results//Figure 3 severity_Germany.pdf"), height = 7,width = 12)
severityplot_DE
dev.off()


jpeg2(here("results//Figure 3 testpos infectiv_Germany.jpeg"), height = 8.5,width = 12)
infectivityplot_DE
dev.off()

jpeg2(here("results//Figure 3 severity_Germany.jpeg"), height = 7,width = 12)
severityplot_DE
dev.off()
